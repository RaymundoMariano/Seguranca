    ...
    public class FormularioService : IFormularioService
    {
        ...
        public async Task UpdateAsync(int formularioId, Formulario formulario)
        {
            try
            {
                ...
                try { await _formularioRepository.UnitOfWork.SaveChangesAsync(); }
                catch (DbUpdateConcurrencyException)
                {
                    if (!_formularioRepository.Exists(formularioId))
                    {
                        throw new ServiceException("Formulario não encontrado - " + formularioId);
                    }
                    throw;
                }
            }
